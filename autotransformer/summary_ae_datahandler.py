...
def greedy_decode(model, encodings):
    #TODO: batch this
    candidates = []
    for candidate in encodings:
        ys = torch.ones(1, 1).fill_(trg_vocab.stoi['<s>']).type_as(src.data)
        for i in range(max_len - 1):
            out = model.decode(candidate.reshape(1,1,300), torch.ones(1,1,1).cuda(), Variable(ys), Variable(subsequent_mask(ys.size(1)).type_as(src.data)))
